...
export default function ReferralCodeSection() {
  ...
  // Show referral code input if user hasn't purchased any plan yet
  if (!hasPurchasedPlan) {
    return <ReferralCodeInput />;
  }

  // Show message if user has purchased plan but no referral code yet
  if (!referralCode) {
    return (
  ...
  // Show referral code management interface
  return (
    <div className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 backdrop-blur-sm p-8 rounded-2xl border border-blue-500/20">
      <div className="flex items-center gap-3 mb-6">
        <Gift className="h-6 w-6 text-blue-500" />
