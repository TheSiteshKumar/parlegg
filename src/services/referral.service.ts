...
export const referralService = {
  ...
  // Get referrer name from user document or auth
  async getReferrerName(userId: string): Promise<string> {
    try {
    ...
    if (!snapshot.empty) {
      ...
      if (referral.status === 'pending') {
        ...
        if (rewards) {
          ...
          // Create reward for referrer (added to earnings wallet)
          ...
          // Create reward for referee (added to earnings wallet)
          await addDoc(collection(db, 'referralRewards'), {
            userId: refereeId,
            type: 'referral',
  ...
  async checkMilestoneBonuses(userId: string): Promise<void> {
    ...
    for (const milestone of MILESTONE_BONUSES) {
      if (completedReferrals >= milestone.referrals) {
        ...
        if (snapshot.empty) {
          // Award milestone bonus (added to earnings wallet)
          await addDoc(collection(db, 'referralRewards'), {
            userId,
            type: 'milestone',
